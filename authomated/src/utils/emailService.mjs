...
import { redisClient, connectRedis } from "../server.mjs";
import { randomUUID } from "crypto";
import { saveTokenToRedis } from './users.mjs';
...
// NOTE: COME BACK TO THIS AS REDIS HELPS CONFIRM TOKEN and no need  for URLSAFETOKEN
...
export const sendVerificationEmail = async (email, userId) => {
  ...
  const verificationLink = `${frontendURL}/api/users/verify/${token}`;
